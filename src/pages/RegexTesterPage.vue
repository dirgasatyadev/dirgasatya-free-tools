...
import { evaluateRegexInWorker, type RegexEvaluation } from '@/composables/useRegexWorker'
...
const evaluation = ref<RegexEvaluation>({ matches: [], replacedText: '', errorMessage: '' })
const isEvaluating = ref(false)
let debounceTimer = 0
let controller: AbortController | null = null
let requestId = 0
watch([pattern, flags, testText, replacement], () => {
  ...
  debounceTimer = window.setTimeout(async () => {
    ...
    try {
      const result = await evaluateRegexInWorker({ pattern: pattern.value, flags: flags.value, text: testText.value, replacement: replacement.value }, 500, controller.signal)
      if (currentRequest === requestId) evaluation.value = result
    } catch (error) {
      if (currentRequest === requestId && !(error instanceof DOMException && error.name === 'AbortError')) {
        evaluation.value = { matches: [], replacedText: '', errorMessage: error instanceof Error ? error.message : 'Regex tidak valid.' }
      }
    } finally { if (currentRequest === requestId) isEvaluating.value = false }
  }, 200)
}, { immediate: true })
...
onBeforeUnmount(() => { window.clearTimeout(debounceTimer); controller?.abort() })
</script>
...
<template>
  <ToolPageShell title="Regex Tester" description="Uji pola JavaScript RegExp, lihat seluruh match dan capture group, serta preview hasil replace." icon="mdi:regex" category="Developer">
    ...
    <p v-if="errorMessage" class="mt-4 rounded-xl bg-rose-50 p-3 text-sm font-semibold text-rose-700 dark:bg-rose-500/10 dark:text-rose-300">{{ errorMessage }}</p><p v-else-if="isEvaluating" class="mt-4 text-sm font-semibold text-indigo-600 dark:text-indigo-300">Mengevaluasi regex di Web Worker…</p>
