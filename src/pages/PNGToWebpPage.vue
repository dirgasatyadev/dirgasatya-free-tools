    ...
    <main class="mx-auto max-w-6xl px-5 py-10 sm:px-8 sm:py-14">
      ...
      <div class="mt-7 grid gap-5 lg:grid-cols-[minmax(0,1fr)_21rem]">
        <section class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-8">
          ...
          <div v-if="items.length && processedCount < items.length && isConverting" class="mt-6 rounded-2xl border border-slate-200 p-4 dark:border-slate-700"><p class="flex items-center gap-2 text-sm font-bold text-sky-700 dark:text-sky-300" role="status"><Icon icon="mdi:loading" class="size-4 animate-spin" aria-hidden="true" /> Memproses otomatis {{ Math.min(processedCount + 1, items.length) }} dari {{ items.length }}...</p><div class="mt-3 h-2 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700" role="progressbar" :aria-valuenow="progressPercentage" aria-valuemin="0" aria-valuemax="100"><div class="h-full rounded-full bg-sky-600 transition-[width] duration-300" :style="{ width: `${progressPercentage}%` }"></div></div></div>
