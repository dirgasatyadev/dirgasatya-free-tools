...
function sample() {
  const xmlSample = '<?xml version="1.0" encoding="UTF-8"?>\n<?app mode="demo"?>\n<article xmlns="https://example.com/article" active="true"><p>Hello <b>world</b>!</p><![CDATA[raw <value>]]><!--preserved--></article>'
...
<template>
  <ToolPageShell :title="title" :description="`Konversikan ${formatName} dan JSON dua arah dengan validasi otomatis, salin, dan download hasil.`" icon="mdi:file-swap-outline" category="Converter">
    ...
    <p v-if="type === 'xml'" class="mt-4 rounded-xl bg-indigo-50 p-3 text-xs leading-5 text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-300">Mode AST preserve-order: urutan mixed content, namespace, atribut, repeated element, CDATA, komentar, dan processing instruction dipertahankan untuk round-trip XML ↔ JSON.</p>
  </ToolPageShell>
</template>
